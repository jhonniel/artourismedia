...
import { postFeaturedImage } from '@/lib/utils'
...
export function LatestInsights({ section, posts }: LatestInsightsProps) {
  ...
  return (
    <section className="relative z-10 -mt-px bg-cream py-16 md:py-24">
      <Container>
        <FadeIn>
                ...
                <Link to={`/insights/${post.slug}`} className="group block h-full">
                  <article className="flex h-full flex-col overflow-hidden rounded-[1.25rem] border border-navy/5 bg-white shadow-card transition-all duration-300 hover:-translate-y-0.5 hover:shadow-elevated sm:rounded-[1.5rem]">
                    <LazyImage
                      ...
                      alt={post.title}
                      wrapperClassName="aspect-[16/10] w-full shrink-0"
                    ...
                    <div className="flex flex-1 flex-col p-4 sm:p-5">
                      ...
                      <h3 className="break-words font-serif text-lg font-normal leading-relaxed text-navy transition-colors group-hover:text-teal">
                        {post.title}
                      </h3>
                      {post.excerpt && (
                      ...
                      <span className="mt-4 inline-flex items-center gap-1 text-xs font-bold uppercase tracking-[0.1em] text-orange transition-colors group-hover:text-orange/80">
                        Read more
