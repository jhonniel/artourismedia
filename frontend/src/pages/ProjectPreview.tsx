...
export default function ProjectPreview() {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['project-preview', uuid, token],
    queryFn: () => endpoints.projectPreview(uuid, token),
  ...
  return (
    <>
      <div className="sticky top-20 z-40 border-b border-orange/20 bg-orange/10 py-2 text-center text-sm font-semibold text-orange">
        ...
        This unpublished project is only visible with a valid preview link.
      </div>
      <SEO title={`Preview: ${data.title}`} description={data.excerpt} noindex />
